...
export const MessageList = ({
    ...
    // Filter messages for display (infinite scroll vs search)
    const messagesToDisplay = useMemo(() => {
        if (searchTerm) {
            return messages; // Show ALL messages when searching
        }
        // Show only the last 'visibleMessagesCount' messages
        return messages.slice(-Math.min(messages.length, visibleMessagesCount));
        ...
        return (
            ...
            >
                ...
                <div className={cn("group relative flex items-end gap-1 min-w-0", isMobile ? "max-w-[calc(100%-3rem)]" : "max-w-[70%]")}>
                    ...
                    <div className={cn("rounded-lg p-3 overflow-hidden min-w-0 break-words shadow-sm", msg.direction === "outbound" ? "bg-[#DCF7C5] text-gray-800 dark:bg-[#044740] dark:text-white" : "bg-white dark:bg-[hsl(var(--chat-customer))] text-gray-800 dark:text-foreground")}>
                        ...
                        {msg.body && msg.message_type !== 'document' && msg.message_type !== 'audio' && <p className="text-sm break-words whitespace-pre-wrap"><HighlightText text={cleanMessageBody(msg.body)} highlight={searchTerm} /></p>}
