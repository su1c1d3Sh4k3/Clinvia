 ...
 *   (rec_<8hex>_msg[1-3]_v<N> por serviço ou rec_default_msg[1-3]_v<N>
 *   padrão da conta — ver _shared/recurrence-meta-template.ts)
...
export function getTemplateKind(name: string | null | undefined): TemplateKind {
    ...
    if (/^rec_(?:default|[0-9a-f]{8})_msg[1-3]_v\d+$/.test(n)) return "recurrence";
    return "custom";
}
