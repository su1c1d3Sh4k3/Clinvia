...
serve(async (req)=>{
  ...
  try {
    ...
    if (webhookData.event === 'messages.upsert') {
      ...
      const { data: existingContact } = await supabase.from('contacts').select('profile_pic_url').eq('remote_jid', remoteJid).single();
      ...
      if (!profilePicUrl) {
        ...
        try {
          ...
          const photoResponse = await fetch(`${evolutionApiUrl}/chat/fetchProfilePictureURL/${encodedInstance}`, {
            method: 'POST',
            headers: {
      ...
      const { data: activeConversations } = await supabase.from('conversations').select('*').eq('contact_id', (await supabase.from('contacts').select('id').eq('remote_jid', remoteJid).single()).data?.id).in('status', [
        'pendente',
        'aberto'
      ]).order('updated_at', {
        ascending: false
