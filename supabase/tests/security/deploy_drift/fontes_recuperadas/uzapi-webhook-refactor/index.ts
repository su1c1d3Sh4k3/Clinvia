...
const mapMessageType = (uzapiType)=>{
  const typeMap = {
...
serve(async (req)=>{
  ...
  try {
    ...
    if (eventType === 'messages_update' && payload.type === 'ReadReceipt') {
      ...
      if (messageIds.length > 0) {
        ...
        for (const messageId of messageIds){
          ...
          const { data, error: updateError } = await supabaseClient.from('messages').update({
            status: status
          }).eq('evolution_id', messageId).select('id');
          if (updateError) {
    ...
    const { data: instance, error: instanceError } = await supabaseClient.from('instances').select('id, apikey, user_id, webhook_url, default_queue_id').eq('instance_name', instanceName).single();
    ...
    if (!userId) {
      ...
      return new Response(JSON.stringify({
        success: false,
        error: "Instance has no user_id configured"
      }), {
        status: 400,
        headers: corsHeaders
      });
    ...
    if (isGroup) {
      ...
      let { data: group, error: groupError } = await supabaseClient.from('groups').select('*').eq('remote_jid', waChatId).single();
      if (!group) {
        console.log('[UZAPI WEHOOK REFACTOR] Group does not exist. Creating...');
        const { data: newGroup, error: createGroupError } = await supabaseClient.from('groups').insert({
          remote_jid: waChatId,
            ...
            if (imagePreviewUrl && imagePreviewUrl.startsWith('http')) {
              try {
                ...
                if (imageResponse.ok) {
                  ...
                  const { error: uploadError } = await supabaseClient.storage.from('avatars').upload(fileName, imageBlob, {
                    contentType: 'image/jpeg',
                    upsert: true
                  });
                  if (!uploadError) {
                    const { data: publicUrlData } = supabaseClient.storage.from('avatars').getPublicUrl(fileName);
                    await supabaseClient.from('groups').update({
                      group_pic_url: publicUrlData.publicUrl
                    }).eq('id', group.id);
                  }
                }
              } catch (e) {
                console.error('[UZAPI WEHOOK REFACTOR] Error processing group image:', e);
              }
            }
          }
        } catch (e) {
          console.error('[UZAPI WEHOOK REFACTOR] Error fetching group details:', e);
        }
      }
      ...
      if (senderPn && group) {
        ...
        // Check/Create Member
        let { data: member, error: memberError } = await supabaseClient.from('group_members').select('*').eq('group_id', group.id).eq('number', senderPn).single();
        if (!member) {
          console.log('[UZAPI WEHOOK REFACTOR] Member does not exist. Creating...');
          const { data: newMember, error: createMemberError } = await supabaseClient.from('group_members').insert({
            group_id: group.id,
          ...
          if (!member.user_id && userId) {
            await supabaseClient.from('group_members').update({
              user_id: userId
            }).eq('id', member.id);
          }
        }
              ...
              if (memberImagePreviewUrl && memberImagePreviewUrl.startsWith('http')) {
                ...
                try {
                  ...
                  if (memImageResponse.ok) {
                    ...
                    const { error: memUploadError } = await supabaseClient.storage.from('avatars').upload(memFileName, memImageBlob, {
                      contentType: 'image/jpeg',
                      upsert: true
                    });
                    if (!memUploadError) {
                  ...
                  console.error('[UZAPI WEHOOK REFACTOR] Error processing member image:', e);
                }
              }
            }
          } catch (e) {
            console.error('[UZAPI WEHOOK REFACTOR] Error fetching member details:', e);
          }
        }
      }
    } else {
      ...
      let { data: contact, error: contactError } = await supabaseClient.from('contacts').select('*').eq('user_id', userId) // Filtrar por tenant
      .eq('number', waNumber).maybeSingle(); // Evita erro se não encontrar
      if (contactError) {
      ...
      if (contact) {
        ...
        console.log('[UZAPI WEHOOK REFACTOR] Contact does not exist. Creating with:', {
          waNumber,
          contactName,
          profilePicUrl
        });
        ...
        const { data: newContact, error: createError } = await supabaseClient.from('contacts').insert({
          number: waNumber,
            ...
            if (imagePreviewUrl && imagePreviewUrl.startsWith('http')) {
              try {
                ...
                if (imageResponse.ok) {
                  ...
                  const { data: uploadData, error: uploadError } = await supabaseClient.storage.from('avatars').upload(fileName, imageBlob, {
                    contentType: 'image/jpeg',
                    upsert: true
                  });
                  if (!uploadError) {
                ...
                console.error('[UZAPI WEHOOK REFACTOR] Exception handling image:', imgError);
              }
            }
          }
        } catch (apiError) {
          console.error('[UZAPI WEHOOK REFACTOR] Error fetching chat details:', apiError);
        }
      }
    }
    ...
    if (contactId || groupId) {
      ...
      let query = supabaseClient.from('conversations').select('*').eq('instance_id', instance.id) // ✅ Obrigatório: filtrar por instância
      ...
      if (conversations && conversations.length > 0) {
        ...
        const newUnreadCount = fromMe ? conversation.unread_count // Não incrementa se mensagem enviada pelo atendente
        ...
        const { data: newConversation, error: createConvError } = await supabaseClient.from('conversations').insert({
      ...
      if (conversation) {
        ...
        if ([
          ...
          'video',
          'document'
        ].includes(messageType) && messageId) {
            ...
            if (downloadResponse.ok) {
              ...
              if (rawBase64) {
                ...
                if (messageType === 'document') {
                  detectedMime = payload.message?.documentMessage?.mimetype || payload.message?.content?.documentMessage?.mimetype || payload.body?.message?.documentMessage?.mimetype;
                }
                if (detectedMime) {
                ...
                const originalPayloadName = payload.message?.documentMessage?.fileName || payload.message?.content?.documentMessage?.fileName || payload.body?.message?.documentMessage?.fileName;
                if (originalPayloadName) {
                  ...
                  const safeName = originalPayloadName.normalize('NFD').replace(/[\u0300-\u036f]/g, "") // Remove accents
                ...
                const { error: uploadError } = await supabaseClient.storage.from('media').upload(fileName, fileBytes, {
        ...
        if (contextInfo) {
          ...
          quotedBody = contextInfo.quotedMessage?.conversation || contextInfo.quotedMessage?.extendedTextMessage?.text || null;
          quotedSender = contextInfo.participant ? contextInfo.participant.includes('@lid') ? 'Atendente' : 'Cliente' : null;
        ...
        const { data: savedMessage, error: msgError } = await supabaseClient.from('messages').insert({
        ...
        if (msgError) {
          ...
          try {
            const { count, error: countError } = await supabaseClient.from('messages').select('*', {
              count: 'exact',
              head: true
          ...
          if (!fromMe && conversation.id) {
            try {
              ...
              const { data: followUp } = await supabaseClient.from('conversation_follow_ups').select('id, category_id, auto_send').eq('conversation_id', conversation.id).single();
              if (followUp && followUp.auto_send) {
                console.log('[UZAPI WEHOOK REFACTOR] Active auto follow up found. Resetting...');
                const { data: templates } = await supabaseClient.from('follow_up_templates').select('time_minutes').eq('category_id', followUp.category_id).order('time_minutes', {
                  ascending: true
                }).limit(1);
                if (templates && templates.length > 0) {
                  const firstTemplateMinutes = templates[0].time_minutes;
                  const nextSendAt = new Date(Date.now() + firstTemplateMinutes * 60 * 1000);
                  await supabaseClient.from('conversation_follow_ups').update({
