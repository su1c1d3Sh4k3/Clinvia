...
serve(async (req)=>{
  ...
  try {
    ...
    const { data: instance, error: fetchError } = await supabaseClient.from('instances').select('*').eq('id', instanceId).single();
    if (fetchError) throw fetchError;
    // Usar nome customizado ou gerar um único
    const instanceName = customName ? customName.toLowerCase().replace(/\s+/g, '_') : `omnichat_${instanceId.slice(0, 8)}_${Date.now()}`;
    ...
    const { error: updateError } = await supabaseClient.from('instances').update({
