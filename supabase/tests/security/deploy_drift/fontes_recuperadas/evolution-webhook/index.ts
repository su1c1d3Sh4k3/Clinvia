...
serve(async (req)=>{
  ...
  try {
    ...
    const { data: instance, error: instanceError } = await supabaseClient.from('instances').select('*').eq('instance_name', instanceName).single();
    ...
    if (event === 'messages.upsert' || event === 'MESSAGES_UPSERT') {
      ...
      const mapMessageType = (evolutionType)=>{
        const typeMap = {
          'conversation': 'text',
      ...
      const base64Content = message.base64 || message.audioMessage?.base64 || message.imageMessage?.base64 || message.videoMessage?.base64 || message.documentMessage?.base64;
      if (base64Content) {
        ...
        try {
          ...
          // Determine MimeType
          const mimetype = message.audioMessage?.mimetype || message.imageMessage?.mimetype || message.videoMessage?.mimetype || message.documentMessage?.mimetype || (message.audioMessage ? 'audio/ogg' : 'application/octet-stream');
          ...
          const { data: uploadData, error: uploadError } = await supabaseClient.storage.from('media').upload(filePath, binary, {
      ...
      if (isGroup) {
        ...
        // 1. Fetch Group Info
        try {
          const groupInfoResponse = await fetch(`${instance.server_url}/group/findGroupInfos/${instanceName}?groupJid=${remoteJid}`, {
            method: 'GET',
            headers: {
              'Content-Type': 'application/json',
        ...
        // 2. Upsert Group
        const { data: group, error: groupError } = await supabaseClient.from('groups').upsert({
        ...
        if (senderJid) {
          ...
          try {
            const senderPicResponse = await fetch(`${instance.server_url}/chat/fetchProfilePictureUrl/${instanceName}`, {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
                'Accept': 'application/json',
          ...
          const { data: member, error: memberError } = await supabaseClient.from('group_members').upsert({
            group_id: groupId,
        ...
        try {
          const contactPicResponse = await fetch(`${instance.server_url}/chat/fetchProfilePictureUrl/${instanceName}`, {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
              'Accept': 'application/json',
        ...
        const { data: contact, error: contactError } = await supabaseClient.from('contacts').upsert({
          remote_jid: remoteJid,
      ...
      if (isGroup) {
        ...
        const { data: existingGroupConv } = await supabaseClient.from('conversations').select('*').eq('group_id', groupId).maybeSingle();
        conversation = existingGroupConv;
      } else {
        // For contacts, find the latest open or pending conversation
        const { data: existingContactConv } = await supabaseClient.from('conversations').select('*').eq('contact_id', contactId).in('status', [
          'open',
          'pending'
        ]).order('updated_at', {
          ascending: false
      ...
      if (conversation) {
        ...
        const { data: updatedConv, error: updateError } = await supabaseClient.from('conversations').update({
        ...
        const { data: newConv, error: insertError } = await supabaseClient.from('conversations').insert({
      ...
      const { error: messageError } = await supabaseClient.from('messages').upsert({
      ...
      // Trigger AI analysis asynchronously (ONLY for inbound messages)
      if (!fromMe) {
        ...
        if (messageType === 'audio' && mediaUrl) {
          ...
          supabaseClient.functions.invoke('transcribe-audio', {
            body: {
              messageId: (await supabaseClient.from('messages').select('id').eq('conversation_id', conversation.id).order('created_at', {
                ascending: false
