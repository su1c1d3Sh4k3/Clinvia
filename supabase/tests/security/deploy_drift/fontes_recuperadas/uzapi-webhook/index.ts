...
serve(async (req)=>{
  ...
  try {
    ...
    const webhookArray = Array.isArray(originalPayload) ? originalPayload : [
      originalPayload
    ];
    ...
    if (body.EventType === 'messages' && body.chat?.wa_isGroup === false) {
      ...
      const { data: instance, error: instanceError } = await supabaseClient.from('instances').select('id, apikey, webhook_url').single();
      if (!instanceError && instance) {
        ...
        const remoteJid = jidCandidates.find((jid)=>jid && typeof jid === 'string' && jid.includes('@s.whatsapp.net') // Ensure it's a valid user JID
        );
        if (!remoteJid) {
          console.error('[ERROR] No valid user JID found. Aborting processing for this message.', {
            candidates: jidCandidates
          });
          return new Response(JSON.stringify({
            success: false,
            error: 'No valid user JID found'
          }), {
            status: 200,
            headers: {
              ...corsHeaders,
        ...
        let { data: contact } = await supabaseClient.from('contacts').select('*').eq('remote_jid', remoteJid).single();
          ...
          if (detailsResponse.ok) {
            ...
            if (contactInfo) {
              ...
              if (!contact) {
                ...
                const { data: updatedContact, error: updateError } = await supabaseClient.from('contacts').update({
                  push_name: pushName,
                  profile_pic_url: profilePic,
                  instance_id: instance.id
        ...
        if (contact) {
          const { data: conversations } = await supabaseClient.from('conversations').select('*').eq('contact_id', contact.id).in('status', [
            'pending',
            'open'
          ]).order('created_at', {
            ascending: false
          ...
          if (conversations && conversations.length > 0) {
            ...
            const { data: newConversation } = await supabaseClient.from('conversations').insert({
