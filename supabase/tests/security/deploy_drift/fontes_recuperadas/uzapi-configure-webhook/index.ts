...
serve(async (req)=>{
  ...
  try {
    ...
    const { data: instance, error: dbError } = await supabase.from("instances").select("apikey").eq("id", instanceId).single();
    if (dbError || !instance?.apikey) {
      console.error("[configure-webhook] DB error:", dbError);
      return new Response(JSON.stringify({
        success: false,
        error: "Instance not found or apikey missing"
      }), {
        status: 404,
        headers: {
          ...corsHeaders,
    ...
    if (!response.ok) {
      return new Response(JSON.stringify({
        success: false,
        error: `UzAPI error: ${response.status}`,
