...
serve(async (req) => {
    ...
    try {
        ...
        if (isGroup) {
            ...
            // Unifica contatos pelos últimos 8 dígitos para evitar duplicação
            // entre formatos: Meta (12 dígitos), UZAPI (13 dígitos + @s.whatsapp.net)
            const rawDigits = waNumber.replace(/@.*$/, '').replace(/\D/g, '');
            ...
            if (conversationIsPending && contactIaOn && iaConfigOn && instanceIaOn && queueIsIa) {
                ...
                try {
                    ...
                    const bdData = await buildBdData(supabase, {
                        ...
                        iaFunnelId,
                        toneInject: (iaConfig as any)?.tone_inject ?? null,
                        crmInstanceId: instance.id,
                        campaignInstanceId: instance.id,
                        bookingInstanceId: instance.id,
                        logPrefix: '[webhook-handle-message]',
                    });
