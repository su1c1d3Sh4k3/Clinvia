...
/**
 * instagram-send-message-sandbox
 *
 * Gêmea de `instagram-send-message` no ambiente de teste. NADA sai pelo
 * Instagram: a resposta da IA vira uma linha em `sandbox_messages` e aparece
 * no chat da página Sandbox da IA. O contrato de payload é o mesmo do nó de
 * produção (`conversation_id`, `message_type`, `message_text`, `audio_base64`,
 * `audio_mime_type`) — no n8n muda só a URL.
 *
 * Diferença de segurança em relação à de produção, de propósito: esta EXIGE
 * `x-api-key`. A de produção não exige nada (o nó do n8n não manda header
 * nenhum), e é justamente por isso que ela não podia continuar sendo a que o
 * fluxo do teste chama.
 *
...
serveMonitored("instagram-send-message-sandbox", async (req) => {
    ...
    try {
        ...
        const text: string | undefined = body!.message_text ?? body!.text;
        const audioBase64: string | undefined = body!.audio_base64;
        const mimeType: string = body!.audio_mime_type || "audio/mpeg";

        if (!text && !audioBase64) {
            return apiError(corsHeaders, {
                status: 400,
                code: "missing_content",
                message: "Nenhum conteúdo para enviar: informe `message_text` (texto) ou `audio_base64` (áudio em base64). Os dois chegaram vazios.",
        ...
        await logSandboxCall(supabase, ctx, {
            function_name: "instagram-send-message-sandbox",
            label: mediaUrl
                ? "IA respondeu com um áudio (fluxo Instagram)"
                : `IA respondeu (fluxo Instagram): "${String(text).slice(0, 80)}"`,
            request: { message_text: text, audio: !!mediaUrl },
        });
        ...
        return unexpectedErrorResponse(corsHeaders, "Falha inesperada na API de envio de mensagem do Instagram no ambiente de teste (instagram-send-message-sandbox)", error, req);
    }
});
