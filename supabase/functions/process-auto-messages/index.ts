...
/**
 * When the Clinvia AI is enabled for a given owner (ia_config.ia_on = TRUE),
 * ALL appointment-related auto messages must be suppressed — the AI handles
 * those communications and firing them again produces duplicate sends.
 * This mirrors the warning banner in the /auto-messages → "Agenda" tab.
 * Cached per invocation to avoid repeated lookups for the same user.
 */
const _iaOnCache = new Map<string, boolean>();
async function isIaOnForUser(supabase: any, userId: string): Promise<boolean> {
  if (_iaOnCache.has(userId)) return _iaOnCache.get(userId)!;
  const { data } = await supabase
    .from("ia_config")
    .select("ia_on")
    .eq("user_id", userId)
    .maybeSingle();
  const enabled = data?.ia_on === true;
  _iaOnCache.set(userId, enabled);
  return enabled;
}

function isAppointmentTriggerType(t: string): boolean {
  return typeof t === "string" && t.startsWith("appointment_");
}
...
async function saveMessage(supabase: any, conversationId: string | null, ownerId: string, body: string) {
...
async function processTargeted(
  ...
  try {
    ...
    // ── IA GUARD: suppress appointment_* auto messages when ia_on=true ──
    if (isAppointmentTriggerType(trigger_type) && await isIaOnForUser(supabase, cfg.user_id)) {
      console.log(`[targeted] skipping ${trigger_type} — IA is ON for user ${cfg.user_id}`);
      return new Response(JSON.stringify({ processed: 0, reason: "ia_enabled_for_owner" }), {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    if (trigger_type === "appointment_created" || trigger_type === "appointment_cancelled") {
      const { data: appt } = await supabase
        .from("appointments")
        .select("id, start_time, contact_id, description, contacts(push_name, number), professionals(name), products_services(name)")
        .eq("id", entity_id)
        .maybeSingle();
      ...
      // Defense-in-depth: skip delivery-automation-created appointments
      if (typeof appt.description === "string" &&
          appt.description.startsWith("Agendado automaticamente via Delivery Automation")) {
        return new Response(JSON.stringify({ processed: 0, reason: "delivery_automation_appointment" }), {
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        });
      }
    ...
    if (trigger_type === "conversation_resolved") {
      ...
      await supabase.from("conversations").update({ nps_sent_at: new Date().toISOString() }).eq("id", conv.id);
...
serveMonitored("process-auto-messages", async (req) => {
  ...
  try {
    ...
    for (const userId of userIds) {
      ...
      // IA GUARD: when ON, skip ALL appointment_* trigger types for this owner.
      const iaOn = await isIaOnForUser(supabase, userId);
      if (iaOn) {
        console.log(`[process-auto-messages] User ${userId} has IA ON — skipping all appointment_* auto messages`);
      }

      try {
        // 1. APPOINTMENT CREATED
        const apptCreatedCfg = userConfigs.find((c: any) => c.trigger_type === "appointment_created");
        if (apptCreatedCfg && !iaOn) {
          ...
          if (instance) {
            ...
            for (const appt of appts) {
              try {
                if (typeof appt.description === "string" &&
                    appt.description.startsWith("Agendado automaticamente via Delivery Automation")) continue;
                ...
                processed++;
              } catch (e: any) { errors.push(`appt_created ${appt.id}: ${e.message}`); }
            }
          }
        }
        ...
        if (!iaOn) {
          for (const reminderType of ["appointment_reminder", "appointment_day_reminder"]) {
            ...
            console.log(`[${reminderType}] janela NOW → ${windowEnd.toISOString()} (${hoursVal}h)`);
            ...
            for (const appt of appts) {
              try {
                ...
                await logSent(supabase, cfg.id, "appointment", appt.id);
                processed++;
              } catch (e: any) { errors.push(`${reminderType} ${appt.id}: ${e.message}`); }
            }
          }
        }
          ...
          if (instance) {
            ...
            for (const appt of appts) {
              try {
                ...
                processed++;
              } catch (e: any) { errors.push(`appt_cancelled ${appt.id}: ${e.message}`); }
            }
          }
        }
          ...
          if (instance) {
            ...
            for (const appt of appts) {
              try {
                ...
                processed++;
              } catch (e: any) { errors.push(`appt_post_service ${appt.id}: ${e.message}`); }
            }
          }
        }
        ...
        for (const cfg of crmEnterCfgs) {
          ...
          for (const deal of deals) {
            try {
              ...
              await logSent(supabase, cfg.id, "deal", deal.id);
              processed++;
            } catch (e: any) { errors.push(`crm_enter ${deal.id}: ${e.message}`); }
          }
        }
        ...
        for (const cfg of crmAfterCfgs) {
          ...
          for (const deal of deals) {
            try {
              ...
              await logSent(supabase, cfg.id, "deal", deal.id);
              processed++;
            } catch (e: any) { errors.push(`crm_after ${deal.id}: ${e.message}`); }
          }
        }
        ...
        for (const cfg of crmStagCfgs) {
          ...
          for (const deal of deals) {
            try {
              ...
              await logSent(supabase, cfg.id, "deal", deal.id);
              processed++;
            } catch (e: any) { errors.push(`crm_stagnation ${deal.id}: ${e.message}`); }
          }
        }

        // 8. SATISFACTION SURVEY (conversation_resolved) — NOT affected by iaOn
        ...
        if (satisfactionCfg) {
          ...
          for (const conv of convs) {
            try {
              ...
              await sendSatisfactionSurvey(conv.contact_id, phone, conv.id, instanceId);
              await supabase.from("conversations").update({ nps_sent_at: now.toISOString() }).eq("id", conv.id);
              await logSent(supabase, satisfactionCfg.id, "conversation", conv.id);
              processed++;
            } catch (e: any) { errors.push(`satisfaction ${conv.id}: ${e.message}`); }
          }
        }

        // 9. PATIENT BIRTHDAY — NOT affected by iaOn
            ...
            if (isInSendWindow(sendHour, sendMinute)) {
              ...
              for (const patient of patients) {
                try {
                  ...
                  processed++;
                } catch (e: any) { errors.push(`birthday ${patient.id}: ${e.message}`); }
              }
            }
          }
        }
