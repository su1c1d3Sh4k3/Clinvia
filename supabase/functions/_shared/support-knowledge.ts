...
export const SUPPORT_TOPICS: SupportTopic[] = [
    {
        ...
        gotchas: [
            ...
            "O custo de IA e o valor cobrado pela Clinbia pelo uso da IA daquela conta. Ele vem da medicao do proprio provedor no projeto da conta — nao e mais uma estimativa por token nem tem desconto de cache para explicar ao cliente.",
            "NUNCA abra a composicao desse valor: nao existe tela, relatorio, exportacao, e-mail ou API que mostre ao cliente o custo de provedor separado nem qualquer percentual de margem. Se pedirem essa abertura, diga que e informacao interna e ofereca o que o cliente ve: valor total, tokens consumidos e o periodo.",
            "Conta que ainda nao tem projeto proprio no provedor aparece com o selo 'estimado': ali o valor e calculado pelo consumo de tokens registrado e pode divergir do fechamento.",
    ...
    },
    {
        ...
        steps: [
            ...
            "Ambiente de teste: em IA > Configuracoes clique em 'Testar IA no ambiente Sandbox'. No topo, preso na tela enquanto se rola a pagina, fica o custo do teste; logo abaixo o chat (escreva como o paciente falaria) com o seletor 'Agenda Real' ou 'Agenda Liberada' e o botao 'Limpar memoria'. Ao lado do chat ha um painel de TRES ABAS: 'Logs' (paciente ficticio editavel, 'O que a IA fez', etapa no CRM e os agendamentos/vendas do teste), 'Voz' (o tom de voz do teste) e 'Simuladores' (campanha, recorrencia, confirmacao de agendamento, venda e convenio). O painel tem a altura do chat e rola por dentro: trocar de aba nao mexe na conversa. Terminou, clique em 'Resetar ambiente'.",
            "Na aba 'Voz' do Sandbox os controles ficam um abaixo do outro: Proximidade, Elaboracao, Assertividade, Abordagem, Formalidade, Expressividade, Tecnicidade, Tratamento e Emoji. O campo 'Sobre a clinica' NAO fica ali: ele e unico e se edita em IA > Tom de voz — o valor salvo la e usado tambem no teste.",
        ],
        gotchas: [
            ...
            "O Direct do Instagram segue as MESMAS regras do WhatsApp, inclusive no follow-up: a conversa precisa estar pendente na fila 'Atendimento IA' e a IA precisa estar ligada NAQUELA CONTA de Instagram (o switch da conta, nao o de um numero de WhatsApp). Desligar a IA da conta para o follow-up dela na hora e devolve as conversas abertas para o atendimento humano.",
    ...
    },
    {
        ...
        resolves: [
            ...
            "uma conexao sumiu da propria tela de Conexoes / foi removida do provedor",
            "perdi as conversas de uma conexao que sumiu da tela",
            "o sistema recusa o nome da conexao nova dizendo que ja foi usado",
        ...
        gotchas: [
            ...
            "Conexao que nao existe mais no provedor (o numero foi apagado la) sai da tela de Conexoes, das listas de escolha (filtros, campanhas, agendamento, escopo de atendente) e do aviso vermelho de desconexao — nao ha o que reconectar, e cobrar reconexao seria um alerta sem saida. O que passou por ela NAO e apagado: conversas, historico de atendimento, campanhas e agendamentos antigos continuam aparecendo com o nome da conexao. Se o cliente diz que 'sumiu tudo', ele esta vendo so a tela de Conexoes — peca para abrir o Atendimento e conferir.",
            "O nome de uma conexao removida continua RESERVADO: criar outra com o mesmo nome e recusado de proposito, porque o roteamento das mensagens que chegam e por nome e duas conexoes homonimas fariam a mensagem cair na errada. Oriente a escolher um nome diferente.",
