-- Rollback da origem declarada nos chamadores SQL.
-- Restaura os corpos EXATOS que estavam em producao antes de
-- 20260923560000 (capturados por `pg_get_functiondef` no momento da geracao).
--
-- Efeito de rodar isto: os 7 voltam a omitir `origem` no payload e
-- `incident_record` volta a inferir e a carimbar `origem_inferida = true`.
-- Nenhum evento ja gravado muda — o campo e decidido na escrita.
--
-- `create or replace` PRESERVA os grants, entao nao ha revoke/grant aqui nem
-- no arquivo de ida.

-- ── admin_simulate_incident ──
