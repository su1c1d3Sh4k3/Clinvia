-- O varredor de tarefas agendadas via SOMENTE o que ainda existe em `cron.job`.
--
-- O bloco B2 fazia `join cron.job j on j.jobid = d.jobid`. `cron.job_run_details`
-- guarda a execucao para sempre; `cron.job` perde a linha no instante em que o
-- job e removido. Resultado: um job que falha e depois e desagendado tem TODO o
-- seu historico de falha apagado do relatorio, em silencio.
--
-- Medido em 23/09, janela de 7 dias: 69 falhas de execucao, das quais 41
-- pertencem a jobids que nao existem mais (22, 4, 5, 10, 32) e por isso nunca
-- foram relatadas.
--
-- O QUE ESTA MIGRATION NAO E: eu diagnostiquei essa cegueira como efeito do RLS
-- de `cron.job` (`cron_job_policy USING (username = CURRENT_USER)`). A policy
-- existe, mas hoje nao esconde nada: rodamos como `postgres` e os 40 jobs
-- visiveis pertencem a `postgres`. Nenhum dos 5 orfaos estava vivo — o mais
-- recente parou de rodar ha 5 horas. A causa e job REMOVIDO, nao job de outro
-- dono. O `left join` cobre os dois casos, mas o ganho medido e menor do que eu
-- afirmei, e isso fica escrito aqui para o proximo que ler o numero.
--
-- Para o relato de job removido nao virar barulho novo, ha um freio: job que nao
-- esta em `cron.job` E cuja execucao e de mais de 30 minutos atras ja e passado,
-- entao entra como `baixa` (fica no painel, nao vai ao telefone). Se ele nao
-- esta cadastrado mas rodou agora ha pouco, entao ou foi removido no meio da
-- pane ou esta mesmo escondido de nos — e ai o placar vale.
--
-- Rollback: 20260923430000_cron_health_left_join_rollback.sql
...
comment on function public.cron_health_scan(integer) is
    'Vigia de tarefas agendadas. Le net._http_response (o cron mente: net.http_post responde ao enfileirar), as execucoes de pg_cron e os jobs sub-horarios que pararam. A severidade de uma falha de execucao vem do placar do job, nao de um valor fixo; varios jobs caindo no mesmo minuto viram um unico incidente de infraestrutura. Falha de job que nao esta mais em cron.job tambem e relatada (left join), como baixa quando a falha ja e antiga.';
