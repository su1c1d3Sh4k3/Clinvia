-- Rollback do item 3 metade SQL (20260923580000).
--
-- Volta as 3 funcoes a engolir. Os fallbacks sao identicos nos dois sentidos —
-- o que se perde ao reverter e so o rastro: `aviso_owner_id`/`aviso_started_at`
-- no context do evento e o `-- motivo:` na excecao de exclusao travada.
--
-- `incident_ingest` volta ao corpo da 20260923570000 (com a origem `ia_n8n`
-- declarada), NAO ao corpo anterior a ela: reverter este item nao pode reverter
-- o item 2 de carona.
--
-- Rodar: npx supabase db query --linked --file supabase/migrations/20260923580000_silencio_sql_registra_rollback.sql

-- ── incident_record (estado anterior) ──
...
-- ── incident_ingest (estado anterior) ──
...
-- ── admin_delete_tenant_data (estado anterior) ──
