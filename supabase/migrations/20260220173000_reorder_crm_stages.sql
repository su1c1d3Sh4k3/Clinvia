-- =============================================
-- Migração: Reordenar estágios Ganho e Perdido
-- Data: 2026-02-20
-- =============================================

-- Parte 1: Atualizar posições nos funis existentes
DO $$
DECLARE
    r RECORD;
BEGIN
    FOR r IN SELECT id FROM public.crm_funnels WHERE name = 'Qualificação Humana' LOOP
        UPDATE public.crm_stages SET position = 999 WHERE funnel_id = r.id AND name = 'Ganho';
        UPDATE public.crm_stages SET position = 1000 WHERE funnel_id = r.id AND name = 'Perdido';
    END LOOP;
END;
$$;
