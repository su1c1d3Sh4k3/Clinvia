-- =====================================================
-- Attendance Metrics: refactor RPC em 2 camadas
-- =====================================================
-- 1. RPC interno get_attendance_metrics_for_owner(owner, start, end):
--    aceita owner_id explicito — reutilizavel pelo wrapper publico E por testes.
-- 2. RPC publico get_attendance_metrics(start, end):
--    resolve auth.uid() → owner_id e delega ao interno.
-- 3. Fix edge case de abandono: usa COALESCE(last_customer_message_at, created_at)
--    para nao marcar conversas novas sem mensagem do cliente como abandonadas.
-- =====================================================
...
BEGIN
    ...
    RETURN public.get_attendance_metrics_for_owner(current_owner_id, p_start, p_end);
END;
$$ LANGUAGE plpgsql SECURITY DEFINER SET search_path = public, pg_temp;
