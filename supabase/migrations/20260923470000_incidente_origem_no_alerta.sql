-- ============================================================
-- Monitoramento — a origem chega ao alerta
--
-- `incident_claim_for_notification` e a fila do despachante: e dela que sai a
-- linha que o `alert-notify` transforma em mensagem. Ela nao devolvia `origem`,
-- entao a coluna criada em 20260923450000 existia no banco e morria antes do
-- WhatsApp — que e onde a pergunta "isso e a IA, o front ou terceiro?" e feita.
--
-- Trocar a lista de colunas devolvidas obriga DROP + CREATE (o Postgres nao
-- deixa `create or replace` mudar o tipo de retorno). DROP apaga os grants, por
-- isso eles sao redeclarados no fim.
--
-- O CORPO E O DA 20260923360000, a definicao mais nova, e nao o da 170000 que
-- criou a funcao. Isto esta escrito porque a primeira tentativa desta migration
-- pegou o corpo da 170000, aplicou sem erro nenhum e desfez tres regras vivas:
-- a espera de analise, a exclusao de `somente_painel` e a gravidade efetiva.
-- Uma funcao recriada de uma versao antiga nao da erro — ela apenas volta no
-- tempo. Quem for mexer de novo: conferir qual e a ultima definicao ANTES.
--
-- As duas colunas entram DEPOIS de `analyzed_at` na assinatura e na mesma
-- posicao no `returning`: a correspondencia e posicional, e trocar a ordem de
-- um lado so faria a origem sair no campo de outra coisa, sem erro nenhum.
-- ============================================================
...
returns table (
    ...
    origem text, origem_inferida boolean,
...
begin
    ...
    returning i.id, i.source, i.component, i.ai_severity, i.ai_summary, i.ai_probable_cause,
              ...
              i.origem, coalesce(i.origem_inferida, true),
              a.kind, a.novas, a.desde;
end;
$$;

comment on function public.incident_claim_for_notification(integer) is
  'Fila de aviso. Reserva a linha (notify_claimed_at) e devolve; quem fecha o ciclo e incident_notification_done. Critica/alta efetivas, sem somente_painel. Desde 20260923470000 devolve tambem origem/origem_inferida, que viram uma linha propria da mensagem.';

-- `create function` concede EXECUTE a PUBLIC; `revoke ... from anon` NAO tira
-- esse grant. Os dois passos, nesta ordem, sempre.
