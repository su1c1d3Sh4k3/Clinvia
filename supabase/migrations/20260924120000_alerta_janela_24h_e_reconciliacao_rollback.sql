-- ROLLBACK de 20260924120000_alerta_janela_24h_e_reconciliacao.sql
--
-- CUIDADO: desfazer isto devolve o defeito que a migration veio consertar —
-- o alerta volta a sair sempre como texto livre, a Meta volta a aceitar fora
-- da janela de 24h e a falhar depois em silencio, e o painel volta a mostrar
-- 'sent' para mensagem que nunca chegou.
--
-- O rollback do codigo tem que vir JUNTO: alert-notify e meta-webhook chamam
-- as funcoes criadas aqui. Reverter so o SQL deixa as duas functions chamando
-- RPC inexistente.
--
-- As colunas ficam por padrao (drop de coluna perde dado e nao e necessario
-- para reverter comportamento). Para remover mesmo, descomente o bloco final.

begin;

drop function if exists public.alert_notification_status(text, text, text, text);
drop function if exists public.alert_recipient_inbound(text);
drop index if exists public.idx_incident_notifications_wamid;

-- canal_alertas_scan volta a versao de 20260923370000 (sem o motivo
-- `sem_confirmacao`, que depende de delivered_at).
...
-- alter table public.alert_recipients       drop column if exists last_inbound_at;
-- alter table public.incident_notifications drop column if exists delivered_at;
