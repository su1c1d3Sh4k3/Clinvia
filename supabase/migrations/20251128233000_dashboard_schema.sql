...
-- 3. RPC: Get Dashboard Stats (Pie Charts)
...
-- 4. RPC: Get Monthly Metrics (Line Charts)
...
-- 5. RPC: Get Team Performance (Table)
...
BEGIN
    ...
    FROM (
        SELECT 
            tm.user_id,
            tm.name,
            -- We don't have avatar_url in team_members, need to fetch from profiles if needed, 
            -- or assume frontend handles it. Let's try to join profiles.
            p.avatar_url,
