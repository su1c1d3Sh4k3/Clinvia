-- Calibragem do cron-health-watch, com os falsos positivos de 23/09 na mao.
--
-- O QUE OS DADOS MOSTRARAM
--
-- Nove incidentes `cron:*` abertos, todos ALTA, todos com event_count = 1.
-- Oito deles nasceram no MESMO minuto (23/09 12:00, alerta as 12:05) e o nono
-- meia hora depois. Oito tarefas sem relacao nenhuma entre si nao quebram
-- juntas: foi UM tropeco de conexao do Postgres, de um minuto, que se curou
-- sozinho — 8 incidentes para 1 evento. Em 7 dias houve 5 minutos assim
-- (22/09 18:00/18:10/18:20/18:30 e 23/09 12:00), sempre "connection failed".
--
-- E o placar de cada um desmente a severidade que receberam:
--   campaign-dispatch-worker ......  3 falhas em 1.440 execucoes (0,2%)
--   process-auto-follow-up ........  3 em 720 (0,4%)
--   openai-provision-worker .......  3 em 285 (1,1%)
--   appointment-confirmation-cron .  3 em 144 (2,1%)
--   conversation-summary-worker ...  1 em 1.440 (0,1%)
--   campaign-expiry ...............  1 em 24
--   incident-scan-db-sources ......  1 em 93
--   reset-stuck-webhook-jobs ......  2 em 48
--   cron-health-watch .............  1 em 63
-- Nenhum deles esta quebrado. Todos voltaram a rodar no ciclo seguinte.
--
-- A CAUSA NO CODIGO
--
-- O bloco B calculava o placar (`falhas_2h`, `ok_2h`, `status_mais_recente`),
-- imprimia no texto do incidente... e chamava `incident_set_severidade_inicial`
-- com 'alta' fixo. O varredor sabia distinguir tropeco de pane e nao usava o
-- que sabia. E nao havia nocao de rajada: cada linha de `job_run_details` virava
-- um incidente proprio.
--
-- O QUE MUDA
--
-- 1. Rajada (>= `cron_health_rajada_jobs` jobs distintos falhando no mesmo
--    minuto) deixa de abrir N incidentes de job e abre UM, em
--    `cron-infra:rajada`. Media quando e um minuto isolado; alta quando se
--    repete dentro da janela de 2h — que e exatamente a diferenca entre o
--    tropeco de 23/09 e a degradacao de 30 minutos de 22/09.
--
-- 2. Falha individual passa a herdar a severidade do placar:
--       nada rodou em 24h e ja sao 3+ tentativas .... alta   (pane)
--       falha unica em 2h, cercada de sucessos, e a
--       ultima execucao foi ok ....................... baixa  (tropeco)
--       o resto ..................................... media
--
-- 3. O piso do catalogo para `cron:` cai de ALTA para MEDIA. Sem isso o item 2
--    seria inutil: o teto de severidade (20260923400000) toma o MAIOR entre a
--    leitura e o piso, entao com piso alta toda falha continuaria alta por fora,
--    por mais baixa que o varredor a classificasse. A pane continua chegando no
--    WhatsApp porque quem grita alta agora e o varredor; e job PARADO (bloco C)
--    segue critica, sem mudanca.
--
-- EFEITO ESPERADO EM PRODUCAO
--   Pelos ultimos 7 dias: 5 rajadas que abririam 28 incidentes passam a abrir 5,
--   e nenhuma falha isolada de job saudavel volta a tocar o telefone. O que
--   ainda toca: job 100% quebrado, job parado, e rajada repetida.
--
-- NAO ENTRA NESTA MIGRATION (achado novo, precisa de aval)
--   `cron.job` tem RLS `(username = CURRENT_USER)`. Tres jobs pertencem a outro
--   dono e somem do `join cron.job` do bloco B — ou seja, o vigia e CEGO para
--   eles: jobid 22 (`instagram-enrich-profiles`, 39 falhas / 0 sucessos em 24h)
--   e jobid 4 e 5 (`check_financial_due_today()` / `check_financial_overdue()`,
--   funcoes que nao existem mais). Trocar o join por `left join` os revela — e
--   abre na hora um alerta ALTA real no telefone. Fica para depois do OK.

-- ── 1. chave editavel do limiar de rajada ───────────────────────────────────
alter table public.llm_platform_settings
    add column if not exists cron_health_rajada_jobs integer not null default 3;

comment on column public.llm_platform_settings.cron_health_rajada_jobs is
    'Quantos jobs distintos precisam falhar no mesmo minuto para o vigia tratar como UM evento de infraestrutura em vez de N falhas de job.';

-- ── 2. piso do catalogo ─────────────────────────────────────────────────────
update public.incident_component_catalog
   set severidade_padrao = 'media',
       acao_padrao = 'Olhe o placar no proprio incidente antes de agir: falha isolada com a ultima execucao ok e tropeco de conexao, nao pane. Pane e quando nao ha nenhum sucesso em 24h. Status "succeeded" nao prova nada: net.http_post responde ao ENFILEIRAR, a falha real aparece no codigo HTTP.',
       updated_at = now()
...
values (
    'cron-infra:', 'prefixo', 'detector',
    'Varios jobs agendados falhando no mesmo minuto. Nao e defeito de nenhum deles: e o banco ou o pool de conexoes tropecando por baixo de todos.',
    'Confira se o minuto foi isolado (tropeco que se curou) ou se ha varios minutos seguidos (degradacao). Em degradacao, olhe conexoes abertas e carga do banco na janela.',
    'media', false, true)
...
-- ── 4. os 9 falsos positivos de 23/09 ───────────────────────────────────────
-- Sao a rajada de 12:00 (8 jobs) mais uma falha isolada de openai-provision-worker
-- as 12:30. Nenhum job esta quebrado. Ficam resolvidos com o motivo escrito.
update public.incidents
   set status = 'resolved',
       resolved_at = now(),
       -- resolved_by e uuid de pessoa; quem fechou aqui foi a migration, e isso
       -- so cabe em notes.
       notes = coalesce(notes || E'\n', '')
               || 'Resolvido pela migration 20260923420000. '
               || 'Falso positivo: tropeco de conexao do Postgres em 23/09 12:00 (8 jobs '
               || 'distintos no mesmo minuto) e uma falha isolada as 12:30. Todos os jobs '
               || 'voltaram a rodar no ciclo seguinte; o pior placar do grupo e 3 falhas '
               || 'em 144 execucoes. A partir desta migration, rajada abre um incidente so '
               || 'e falha isolada de job saudavel nasce baixa.'
 where status <> 'resolved'
   and component like 'cron:%'
   and event_count = 1
