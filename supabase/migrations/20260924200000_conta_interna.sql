-- Conta interna: existe para ser medida POR NOS, e nao para entrar nas medidas.
--
-- A sentinela de login precisa de uma conta de verdade para fazer um grant de
-- senha de verdade a cada 5 minutos. Sem uma marca, essa conta entraria em tudo
-- que a plataforma conta: ela e um cliente ativo que nunca troca mensagem, nunca
-- consome IA e nunca vai ter projeto na OpenAI. Cada um desses "nunca" e uma
-- condicao que um detector nosso trata como defeito — e o resultado seria a
-- sentinela poluindo exatamente os numeros que ela existe para proteger.
--
-- `profiles.is_internal` marca a conta como infraestrutura. Ela continua
-- aparecendo na lista de clientes do Super Admin (esconder seria pior: ninguem
-- lembraria que ela existe), mas sai de quatro lugares:
--
--   1. provisionamento de projeto na OpenAI  (trigger enqueue_openai_provision)
--   2. os tres detectores de provisionamento (provisionamento_scan)
--   3. zeragem e anomalia de custo           (openai_alert_scan)
--   4. contagem de contas e tenant ocioso    (admin_get_dashboard_metrics)
--
-- O item 2 e o que mais importa: sem ele, tirar a conta do provisionamento a
-- transformaria em "conta ativa sem chave da OpenAI" — um incidente ALTA a cada
-- 15 minutos, para sempre, por causa de uma decisao que foi nossa.
...
comment on column public.profiles.is_internal is
    'Conta de infraestrutura nossa (ex.: a que a sentinela de login usa). Fica FORA de provisionamento da OpenAI, dos detectores de provisionamento, dos alertas de consumo e das contagens do painel. Nao esconde a conta da lista.';

-- 1. Provisionamento: nao enfileira -------------------------------------------
...
as $function$
begin
  -- 0. Conta de infraestrutura nao tem IA e nao tem projeto proprio. Precisa vir
  --    antes de tudo: quem nao e cliente nao entra em nenhuma das regras abaixo.
...
-- 2. Detectores de provisionamento: nao acusam a ausencia que nos causamos -----
...
-- Hoje a conta interna ja cairia fora sozinha, por nao ter `openai_project_id`.
-- O filtro explicito existe para que isso continue verdade no dia em que alguem
-- provisionar um projeto para ela a mao — a exclusao e da CONTA, nao um efeito
-- colateral de ela estar sem projeto.
...
-- Contagem de contas e "tenant ocioso". O ocioso e o que doeria: a conta da
-- sentinela nunca troca uma mensagem, entao ela apareceria todo dia na lista de
