-- Rollback de 20260923440000_cron_http_placar.sql.
--
-- Devolve o cron_health_scan ao corpo de 20260923430000 (bloco A sem placar,
-- severidade so pelo codigo HTTP, retencao de cron_http_calls em 2h) e devolve o
-- piso do prefixo cron-http: para alta.
--
-- As colunas status_code/checked_at e o indice NAO sao removidos de proposito:
-- sao aditivos, ninguem mais depende deles e apaga-los joga fora o unico registro
-- de desfecho que sobrevive a poda de 30 min de net._http_response. Se for mesmo
-- preciso, o drop esta comentado no fim.
...
-- Remocao das colunas (so se realmente necessario):
-- drop index if exists public.idx_cron_http_calls_alvo_created;
-- alter table public.cron_http_calls drop column if exists status_code, drop column if exists checked_at;
