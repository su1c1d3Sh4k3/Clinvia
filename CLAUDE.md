...
- `supabase.functions.invoke` on a non-2xx replaces the message with the fixed string `"Edge Function returned a non-2xx status code"`, returns `data: null` and hides the body in `error.context`. Always unwrap through `src/lib/functionError.ts` (`mensagemDoErroDaFuncao`) — `if (data?.error)` never fires
  - **`error.context` is a `Response`, and a `Response` body can only be read ONCE.** Whoever read it twice — reason first, then raw body for the log — got the reason and then `null`, and concluded the function had explained nothing. It is the diagnostic tool lying about the diagnosis. `mensagemDoErroDaFuncao` reads it once via `clone()`; never call `.json()`/`.text()` on `error.context` yourself
